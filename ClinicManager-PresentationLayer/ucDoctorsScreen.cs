...
{
    public partial class ucDoctorsScreen : UserControl
    {
        private string _Input, _FilteringMethod = "All Fields";

        public ucDoctorsScreen()
        {
            InitializeComponent();
        }

        private void ucDoctorsScreen_Load(object sender, EventArgs e)
        {
            BaseScreen.ConfigureHeaderBar("Doctors Screen");
        ...
        {
            enAllFields, enName, enSpecialization, enPhone, enGender,
            enBirthDateFromYear, enBirthDateYear, enAddress, enEmail
        }
        ...
        private Dictionary<_EnFilteringMethods, string> _FilteringMethodsDictionary
            = new Dictionary<_EnFilteringMethods, string>()
            {
                ...
                { _EnFilteringMethods.enSpecialization, "Specialization" },
            ...
            string arg3)
        {
            ...
            switch (_FilteringMethodName(_FilteringMethod))
            {
                ...
                case _EnFilteringMethods.enName:
                    ...
                    break;
                case _EnFilteringMethods.enSpecialization:
                    BaseScreen.ViewData(clsDoctor.FilterBySpecialization(_Input));
                    break;
                case _EnFilteringMethods.enPhone:
                    BaseScreen.ViewData(clsDoctor.FilterByPhone(_Input));
                    break;
                case _EnFilteringMethods.enGender:
                    ...
                    BaseScreen.ViewData(clsDoctor.FilterByGender(_Input));
                    break;
                case _EnFilteringMethods.enBirthDateFromYear:
                    ...
                    BaseScreen.ViewData(
                        clsDoctor.FilterByBirthDateFrom(BirthDateFromYear)
                    );
                    break;
                case _EnFilteringMethods.enBirthDateYear:
                    ...
                    BaseScreen.ViewData(
                        clsDoctor.FilterByBirthDateYear(BirthDateFrom, BirthDateTo)
                        );
                    break;
                case _EnFilteringMethods.enAddress:
                    BaseScreen.ViewData(clsDoctor.FilterByAddress(_Input));
                    break;
                case _EnFilteringMethods.enEmail:
                    BaseScreen.ViewData(clsDoctor.FilterByEmail(_Input));
                    break;
                default:
                    BaseScreen.ViewData(clsDoctor.GetAllDoctors());
                    break;
        ...
        {
            frmDoctorInfoScreen frm = new frmDoctorInfoScreen();
        ...
        private void BaseScreen_OnEditRowSelected(int obj)
        {
            ...
            {
                frmDoctorInfoScreen frm = new frmDoctorInfoScreen(DoctorID);
                frm.ShowDialog();
            }
            else
            {
                MessageBox.Show("The selected doctor does not exist.", "Error",
                    MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            BaseScreen_OnApplyFiltering(_Input, _FilteringMethod, "");
        }
        ...
        private void BaseScreen_OnDeleteRowSelected(int obj)
        {
            ...
            {
                bool isDeleted = clsDoctorServices.DeleteDoctor(DoctorID);
                if (isDeleted)
                {
